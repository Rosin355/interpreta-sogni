...
export const PaywallBlur = ({ hiddenPreview, dreamId, isAuthenticated = false }: PaywallBlurProps) => {
  ...
  return (
    <div className="mt-6">
      {/* Teaser sfocato — flusso normale, niente overlay sopra */}
      <div
        aria-hidden
        className="select-none pointer-events-none max-h-32 overflow-hidden text-sm leading-relaxed text-foreground/70"
        style={{
          filter: "blur(6px)",
          WebkitMaskImage:
            "linear-gradient(to bottom, hsl(0 0% 0% / 1) 0%, hsl(0 0% 0% / 0.6) 60%, hsl(0 0% 0% / 0) 100%)",
          maskImage:
            "linear-gradient(to bottom, hsl(0 0% 0% / 1) 0%, hsl(0 0% 0% / 0.6) 60%, hsl(0 0% 0% / 0) 100%)",
        }}
      >
      ...
      {/* Bridge gradient: collega elegantemente teaser e CTA */}
      <div
        className="h-12 -mt-8 relative z-10 pointer-events-none"
        style={{
          background:
            "linear-gradient(to bottom, transparent 0%, hsl(var(--background) / 0.7) 50%, hsl(var(--background)) 100%)",
        }}
      />

      {/* CTA block — flusso normale, sotto al teaser */}
      <div className="text-center max-w-md mx-auto px-2 pt-2">
        <div
