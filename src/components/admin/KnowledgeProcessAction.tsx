...
// Admin-only action: run process-knowledge-source in dry_run, then (on confirm)
// in process mode. No AI providers, no embeddings — chunks are inserted with
// embedding = null and the source stays in a non-active status (draft).
//
// Safety: uses the existing authenticated Supabase client session (the JWT is
// attached automatically by supabase.functions.invoke). Only counts / lengths
// are shown — never raw_text or chunk content.
...
/** Pull a safe code/message out of a functions.invoke error. Never content. */
...
const KnowledgeProcessAction = ({ sourceId, title, onProcessed }: Props) => {
  ...
  return (
    <>
      ...
      >
        <Cog className="h-3.5 w-3.5" /> Processa fonte
      </Button>

      <Dialog
        open={open}
        onOpenChange={(o) => {
          if (busy) return; // don't close mid-call
          setOpen(o);
          if (!o) reset();
        }}
      >
...
export default KnowledgeProcessAction;
